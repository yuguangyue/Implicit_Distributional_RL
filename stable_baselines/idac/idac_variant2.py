...
class IDAC(OffPolicyRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=3e-4, buffer_size=50000,
                 ...
                 tau=0.005, ent_coef='auto', target_update_interval=1,noise_dim=5,noise_num=51,
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("model", reuse=False):
                    ...
                    deterministic_action, policy_out_, logp_pi, *_ = self.policy_tf.make_actor(self.processed_obs_ph, self.noises_ph)
                    
                    logp_pi = tf.reduce_mean(tf.reshape(logp_pi,(-1,self.noise_num)),axis=1, keep_dims = True)
                    ...
                    qf1, _, _ = self.policy_tf.make_critics(self.processed_obs_ph,  self.actions_ph, self.noises_ph,
                                                                     create_qf=True, create_vf=False)

                    _, qf2, _ = self.policy_tf.make_critics(self.processed_obs_ph,  self.actions_ph, self.noises_ph_plus,
                    ...
                    qf1_pi = tf.reduce_mean(tf.reshape(qf1_pi,(-1,self.noise_num)),axis=1, keep_dims = True)
                    qf2_pi = tf.reduce_mean(tf.reshape(qf2_pi,(-1,self.noise_num)),axis=1, keep_dims = True)
    ...
    def _train_step(self, step, writer, learning_rate, dis_eval_array, dis_eval_interval, dis_path):
        ...
        idx = np.arange(0, self.batch_size*(self.noise_num), self.noise_num)
        ...
        obs_tile = np.reshape(np.tile(batch_obs, [1, self.noise_num]), (-1,n_s)) 
